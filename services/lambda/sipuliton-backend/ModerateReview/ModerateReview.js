    ...
    try {
        ...
        if (event.body !== null && event.body !== undefined) {
            ...
            if (body.action == "accept"){
                ...
                newStatus = 1
                var acceptLogQuery = `
                INSERT INTO review_accept_log (review_id, accepter_id, accepted)
                VALUES ($1, $2, $3)`
                const resAccept = await client.query(
                    acceptLogQuery, [body.review_id, body.moderatorId, currentTime]);

            } else if (body.action == "reject"){
                console.log("Review was rejected")
                newStatus = -1
                var rejectLogQuery = `
                INSERT INTO review_reject_log (review_id, rejecter_id, rejected, reason)
                VALUES ($1, $2, $3, $4)`
                const resReject = await client.query(
                    rejectLogQuery, [body.review_id, body.moderatorId, currentTime, body.reason]);
            ...
            var updateQuery = "UPDATE review SET status=$1 WHERE review_id=$2";
            var resUpdate =  await client.query(
                updateQuery, [newStatus, body.review_id]);
            await client.end();
